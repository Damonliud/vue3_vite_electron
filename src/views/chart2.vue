...
let processDataByQuarter = (dataObject) => {
  const quarterSums = new Map();
  for (const key in dataObject) {
    if (Object.prototype.hasOwnProperty.call(dataObject, key)) {
      const subArray = dataObject[key];

      const quarterMatch = key.match(/^(Quarter_\d+)/);
      if (!quarterMatch) {
        console.warn(`警告: 键 "${key}" 不符合预期的 "Quarter_X" 格式，已跳过。`);
        continue;
      }
      const quarterLabel = quarterMatch[1];

      const currentSubArrayWeightedVelocitySum = subArray.reduce((sum, item) => {
        const wv = Number(item.weightedVelocity);
        return sum + (isNaN(wv) ? 0 : wv);
      }, 0);

      const currentQuarterTotal = quarterSums.get(quarterLabel) || 0;
      quarterSums.set(quarterLabel, currentQuarterTotal + currentSubArrayWeightedVelocitySum);
    }
  }

  const resultArray = [];
  for (const [quarterLabel, totalWeightedVelocity] of quarterSums.entries()) {
    resultArray.push({
      value: Number(totalWeightedVelocity.toFixed(2)),
      label: quarterLabel
    });
  }
